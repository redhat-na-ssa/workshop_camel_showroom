...
This application will automatically pick up the existing CDC messages from the Kafka topic. So as soon as you start it, you should see the log messages showing as such. If everything went well, you should be able to see information populated in the http://cashback.{user}-globex.{openshift_cluster_ingress_domain}[Cashback page].
