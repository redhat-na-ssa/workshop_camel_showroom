...
In a new browser tab, open the Order Simulator app at http://replaceme.{user}-globex.{openshift_cluster_ingress_domain}/replacemetoo. Simulate a few orders using the default JSON request. If everything is working properly, you should see the log messages from your Camel route showing that messages are being picked up and processed without error.
